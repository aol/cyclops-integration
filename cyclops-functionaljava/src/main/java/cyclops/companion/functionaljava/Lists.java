...
public class Lists {
    ...
    public static  <T,R> List<R> tailRecEither(T initial, Function<? super T, ? extends List<? extends cyclops.control.Either<T, R>>> fn) {
        List<cyclops.control.Either<T, R>> next = List.arrayList(cyclops.control.Either.left(initial));
        ...
        for(;;){
            ...
            next = next.bind(e -> e.visit(s -> {
                        ...
                        newValue[0]=true;
                        return (List<cyclops.control.Either<T,R>>)fn.apply(s);
    ...
    public static  <W1,T> Coproduct<W1,list,T> coproduct(InstanceDefinitions<W1> def1,T... values){
        return coproduct(List.list(values),def1);
    }
        ...
        public static <T,R> MonadRec<list> monadRec(){
            return new MonadRec<list>() {
                ...
                public <T, R> Higher<list, R> tailRec(T initial, Function<? super T, ? extends Higher<list, ? extends Either<T, R>>> fn) {
                    return widen(Lists.tailRecEither(initial,fn.andThen(ListKind::narrow)));
                }
            };
        }
        /**
