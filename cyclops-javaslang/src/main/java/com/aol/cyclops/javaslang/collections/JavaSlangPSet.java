...
public class JavaSlangPSet<T> extends AbstractSet<T>  implements PSet<T> {
     ...
     * <pre>
     * {@code 
     * PSet<Integer> q = JavaSlangPSet.<Integer>toPSet()
    ...
    public static <T> Reducer<PSet<T>> toPSet() {
        return Reducer.<PSet<T>> of(JavaSlangPSet.empty(), (final PSet<T> a) -> b -> a.plusAll(b), (final T x) -> JavaSlangPSet.singleton(x));
    }
