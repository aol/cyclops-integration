...
public class LazyPersistentSetXTest extends AbstractCollectionXTest  {
    ...
    public <T> FluentCollectionX<T> of(T... values) {
        PersistentSetX<T> list = DexxHashSetX.empty();
        for (T next : values) {
            list = list.plus(next);
        }
    ...
    public void onEmptySwitch() {
        assertThat(DexxHashSetX.empty()
                          .onEmptySwitch(() -> PersistentSetX.of(1, 2, 3)).toList(),
                   equalTo(DexxHashSetX.of(1, 2, 3).toList()));
    }
    ...
    public FluentCollectionX<Integer> range(int start, int end) {
        return DexxHashSetX.range(start, end);
    }

    @Override
    public FluentCollectionX<Long> rangeLong(long start, long end) {
        return DexxHashSetX.rangeLong(start, end);
    }

    @Override
    public <T> FluentCollectionX<T> iterate(int times, T seed, UnaryOperator<T> fn) {
        return DexxHashSetX.iterate(times, seed, fn);
    }

    @Override
    public <T> FluentCollectionX<T> generate(int times, Supplier<T> fn) {
        return DexxHashSetX.generate(times, fn);
    }

    @Override
    public <U, T> FluentCollectionX<T> unfold(U seed, Function<? super U, Optional<Tuple2<T, U>>> unfolder) {
        return DexxHashSetX.unfold(seed, unfolder);
    }
    @Test
