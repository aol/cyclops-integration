...
public class LazyListXCoreTest extends AbstractOrderDependentCollectionXTest {
    ...
    public void onEmptySwitch(){
        assertThat(LazyListX.empty().onEmptySwitch(()->LazyListX.of(1,2,3)),equalTo(LazyListX.of(1,2,3)));
    }
    @Test
    public void multipaths() {

        ListX<Integer> list = LazyListX.of(1, 2, 3);
    ...
    public void flatMapPublisherWithAsync20k() throws InterruptedException {
        for (int x = 0; x < 10_000; x++) {
            assertThat(ReactiveSeq.generate(() -> 1)
                    .flatMapPublisher(i -> Maybe.of(i), 500, QueueFactories.unboundedQueue()).limit(300_000).toList(()->LazyListX.of()),
    ...
    public FluentCollectionX<Integer> range(int start, int end) {
        return LazyListX.range(start, end);
    }

    @Override
    public FluentCollectionX<Long> rangeLong(long start, long end) {
        return LazyListX.rangeLong(start, end);
    }

    @Override
    public <T> FluentCollectionX<T> iterate(int times, T seed, UnaryOperator<T> fn) {
        return LazyListX.iterate(times, seed, fn);
    }

    @Override
    public <T> FluentCollectionX<T> generate(int times, Supplier<T> fn) {
        return LazyListX.generate(times, fn);
    }

    @Override
    public <U, T> FluentCollectionX<T> unfold(U seed, Function<? super U, Optional<Tuple2<T, U>>> unfolder) {
        return LazyListX.unfold(seed, unfolder);
    }
