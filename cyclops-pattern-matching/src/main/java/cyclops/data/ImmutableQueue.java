...
public interface ImmutableQueue<T> extends Sealed2<ImmutableQueue.Some<T>,ImmutableQueue.None<T>>,
    ...
    default ImmutableQueue<T> replace(T currentElement, T newElement){
        ...
        while(!tail.isEmpty()){
            ...
            Tuple3<ImmutableQueue<T>, ImmutableQueue<T>, Boolean> t3 = tail.match(c -> {
    ...
    default ImmutableQueue<T> removeFirst(Predicate<? super T> pred){
        ...
        ImmutableQueue<T> ar = rem.match(s -> s.match((h, t) -> t), n -> n);
    ...
    default Seq<T> imSeq(){
        ...
        return match(c->Seq.cons(c.head(),c.tail().imSeq()), nil->Seq.empty());
    }
    ...
    default Zipper<T> focusAt(int pos, T alt){
        ...
        ImmutableQueue<T> right= t2.v2.match(c->c.tail(), n->null);
